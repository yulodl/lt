...
var threeSumClosest = function(nums, target) {
    ...
    for (let i = 0; i < len - 2; i++) {
        ...
        let k = len - 1;
        while (j < k) {
            const nextClosest = nums[i] + nums[j] + nums[k];
            if (closest === null || Math.abs(target - nextClosest) < Math.abs(target - closest)) {
                closest = nextClosest;
            }

            // console.log(i, j, k, nextClosest, closest);

            if (nextClosest === target) {
                return target;
            }
